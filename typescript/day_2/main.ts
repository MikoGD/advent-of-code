...
const input_puller_2 = (file_name: string): number[] => {
  const input = fs_2.readFileSync(file_name, 'utf-8')
  const code_string = input.split(',')
  let code = code_string.map((val: string) => parseInt(val, 10))

  return code
}

const run_opcode = (code: number[], instruction_pointer: number): number[] => {
  let temp_code = code
...
const run_program = (code: number[]): number[] => {
  let temp_code = code
...
const get_solution_2 = (): void => {
  const code = input_puller_2('./day_2/input.txt')
  let temp_code: number[]
  ...
  for (let noun = 0; noun < 100; noun++) {
    for (let verb = 0; verb < 100; verb++) {
      ...
      if (temp_code[0] == 19690720) {
        ...
        console.group(
          `(100 * noun) + verb = ${100 * temp_code[1] + temp_code[2]}`
        )
        return
      }
    }
